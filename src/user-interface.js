...
const playTransition = function(scene, callback) {
  // Hide UI overlay
  scene.scene.setVisible(false, 'InventoryManager');

  let w = scene.registry.values.windowWidth;
  let h = scene.registry.values.windowHeight;

  let ellipse = new Phaser.Geom.Ellipse(w / 2, h / 2, w, h);
  ...
  scene.cameras.main.setMask(
    new Phaser.Display.Masks.BitmapMask(scene, scene.mask)
  );
  ...
  let expand = function() {
    ...
    scene.add.tween({
      ...
      onComplete: () => {
        ...
        scene.mask.destroy();
        scene.scene.setVisible(true, 'InventoryManager');
      }
    });
  }
